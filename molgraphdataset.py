...
"""
NamedTuple for representing graph data.

Parameters:
- n_nodes (int): The number of nodes in the graph.
- node_features (Tensor): Tensor representing node features.
- edge_features (Tensor): Tensor representing edge features.
- edge_index (Tensor): Tensor representing edge indices.

NamedTuple 'GraphData' is used to encapsulate key information about a graph, making it easier to work with graph data in a structured way. It includes the following fields:
- n_nodes: The total number of nodes in the graph.
- node_features: A tensor containing node-specific features.
- edge_features: A tensor containing edge-specific features.
- edge_index: A tensor representing the edge indices, typically in COO format.

Example Usage:
# Create a GraphData instance for a graph with 10 nodes.
graph = GraphData(
    n_nodes=10,
    node_features=torch.randn(10, 64),  # 10 nodes with 64-dimensional features
    edge_features=torch.randn(20, 32),  # 20 edges with 32-dimensional features
    edge_index=torch.tensor([[0, 1, 2, ...], [1, 2, 3, ...]]),  # Edge indices
)

# Access graph properties using named fields.
print("Number of nodes:", graph.n_nodes)
print("Node feature tensor:", graph.node_features)
print("Edge feature tensor:", graph.edge_features)
print("Edge index tensor:", graph.edge_index)
"""
...
class PairData(Data):
    """
    Represents pairs of graphs with associated data.

    This class is adapted from https://pytorch-geometric.readthedocs.io/en/latest/advanced/batching.html#pairs-of-graphs

    Args:
        edge_index_r: Edge indices for the reference graph.
        x_r: Node features for the reference graph.
        edge_index_p: Edge indices for the paired graph.
        x_p: Node features for the paired graph.
        edge_feat_r: Additional edge features for the reference graph.
        edge_feat_p: Additional edge features for the paired graph.
        y_r: Labels or target values for the reference graph.
        y_p: Labels or target values for the paired graph.
        eq_as: Equivalence assignments (for pairing nodes in source graph).
    """
