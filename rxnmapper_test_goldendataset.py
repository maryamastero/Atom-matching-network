...
def get_atom_correspondance(gt_dataset, pred_data_set, reaction_ind):
...
def get_symmetry_gt(gt_dataset, pred_data_set, reaction_ind):

    correspondace ,eq_as= get_atom_correspondance(gt_dataset, pred_data_set, reaction_ind)
    symmetry_gt = []
...
def get_predictions_react2prod(pred_data_set, reaction_ind):
    rxn = pred_data_set.mapped_rxn.iloc[reaction_ind]
    reactants_rxn, products_rxn = get_reaction_componant(rxn)
    r_mol_rxn = Chem.MolFromSmiles(reactants_rxn)
    p_mol_rxn = Chem.MolFromSmiles(products_rxn) 
...
if __name__ == '__main__':
    ...
    for reation_ind in range(185):
            predictions =get_predictions_react2prod(rxn_csv, reation_ind)
            symmetry_gt = get_symmetry_gt(test_data, rxn_csv, reation_ind)
            acc = calculate_accuracy(symmetry_gt, predictions)
            all_acc.append(acc)
